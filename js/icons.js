/**
 * Icon set — a small, self-hosted subset of Lucide (https://lucide.dev, ISC License).
 * Generated from lucide-static so the site needs no icon CDN or library.
 *
 *  - "instagram" comes from Lucide v0.460 (brand icons were removed from later versions).
 *  - "whatsapp" is adapted from Tabler Icons (MIT License, https://tabler.io/icons),
 *    drawn in the same 24px / round-stroke style, because Lucide ships no brand logos.
 *
 * Usage in HTML:  <i data-lucide="phone" data-size="20"></i>   → call hydrateIcons()
 * Usage in JS:    icon("phone", { size: 20 })                  → returns an <svg> string
 * To add an icon: copy the inner markup of any icon from https://lucide.dev into ICONS.
 */
...
export function icon(name, { size = 20, strokeWidth = 1.75, className = "", label = "" } = {}) {
  ...
  return `<svg class="icon icon-${name}${className ? " " + className : ""}" xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" ${a11y}>${markup}</svg>`;
}

/** Replaces every <i data-lucide="name"> inside root with the matching inline SVG. */
export function hydrateIcons(root = document) {
  root.querySelectorAll("[data-lucide]").forEach((el) => {
    const svgMarkup = icon(el.getAttribute("data-lucide"), {
      size: el.getAttribute("data-size") || 20,
      strokeWidth: el.getAttribute("data-stroke") || 1.75,
      className: el.getAttribute("class") || "",
      label: el.getAttribute("aria-label") || "",
    });
    if (!svgMarkup) return;
    const tpl = document.createElement("template");
    tpl.innerHTML = svgMarkup;
